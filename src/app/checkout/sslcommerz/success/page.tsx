    ...
    useEffect(() => {
        const processPayment = async () => {
            try {
                ...
                if (!val_id) {
                    setStatus('failed');
                    setMessage('Invalid payment response. Missing validation ID.');
                    return;
                }
                ...
                if (!pendingOrderStr) {
                    setStatus('failed');
                    setMessage('Order data not found. Please contact support if your account was charged.');
                    return;
                }
                ...
                const validateResponse = await fetch('/api/sslcommerz/validate', {
                    method: 'POST',
                    headers: {
                ...
                const validatedPayment = await validateResponse.json();

                if (validatedPayment.status !== 'VALID' && validatedPayment.status !== 'VALIDATED') {
                    setStatus('failed');
